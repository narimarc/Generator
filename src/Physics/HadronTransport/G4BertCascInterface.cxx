...
G4BertCascInterface::G4BertCascInterface()
: EventRecordVisitorI("genie::G4BertCascInterface")
{
  InitG4Particles();
}

//___________________________________________________________________________
G4BertCascInterface::G4BertCascInterface(string name)
: EventRecordVisitorI("genie::G4BertCascInterface")
{
  InitG4Particles();
}
//___________________________________________________________________________
G4BertCascInterface::G4BertCascInterface(string name, string config)  :
...
//___________________________________________________________________________
int G4BertCascInterface::G4BertCascade(GHepRecord * evrec) const{
...
//___________________________________________________________________________
double G4BertCascInterface::GenerateStep(GHepRecord*  /*evrec*/, GHepParticle* p) const{
...
void G4BertCascInterface::ProcessEventRecord(GHepRecord* evrec) const
{
  ...
  LOG("G4BertCascInterface", pINFO) << " Inappropriate event generation mode - exit ";
  return;
}

LOG("G4BertCascInterface", pINFO) << "Done with this event";
}

//___________________________________________________________________________
void G4BertCascInterface::InitG4Particles() const
{
...
//___________________________________________________________________________
void G4BertCascInterface::GenerateVertex(GHepRecord * evrec) const
{
...
//___________________________________________________________________________
void G4BertCascInterface::SetTrackingRadius(const GHepParticle * p) const
{
  ...
  LOG("G4BertCascInterface", pNOTICE) 
  << "Setting tracking radius to R = " << fTrackingRadius;
}

//___________________________________________________________________________
bool G4BertCascInterface::IsInNucleus(const GHepParticle * p) const
{
...
//___________________________________________________________________________
void G4BertCascInterface::TransportHadrons(GHepRecord * evrec) const
{
...
//  std::cout << " sepEnergy = " << sepEnergy << std::endl;
  ...
  TObjArrayIter piter(evrec);
  int icurr(0);
  bool has_incidenBaryon(false);
  std::vector<GHepParticle> List_of_secondaries, List_of_transparents;
  bool has_secondaries(false), has_remnant(false);
  fRemnA=remNucl->A();
  fRemnZ=remNucl->Z();
  while( (p = (GHepParticle*) piter.Next()) ) {
    icurr++;
    ...
    if(!this->NeedsRescattering(sp)) continue;
  ...
  if (!has_incidenBaryon && p->Status() == kIStHadronInTheNucleus) {
...
TLorentzVector remX(tgtNucl->Vx(), tgtNucl->Vy(), tgtNucl->Vz(), tgtNucl->Vt() );
  ...
  if (Nfrag > 0) {
    // Get largest nuclear fragment in output and call it the remnant
//std::cout<<" Number of outgoing nuclei= "<<Nfrag<<"\n";
  ...
  for (int l = 0; l < Nhad; l++) {
    ...
    GHepParticle new_particle(npdg, kIStStableFinalState, rem_nucl, -1,-1,-1,tempP, remX);
    evrec->AddParticle(new_particle);
  }
  has_remnant=true;
}
...
TLorentzVector G4BertCascInterface::pincident(std::vector<GHepParticle> partList)const{
  TLorentzVector pIncident;
  for(size_t it=0;it<partList.size();it++){
    pIncident+=*(partList.at(it).P4());
  }
  return pIncident;
}
bool G4BertCascInterface::NeedsRescattering(const GHepParticle * p) const {
...
//___________________________________________________________________________
G4ParticleDefinition* G4BertCascInterface::PDGtoG4Particle(int pdg) const
{
  ...
  switch(pdg) { 
    ...
    default:
    LOG("G4BertCascInterface::PDGtoG4Particle", pWARN)
...
//___________________________________________________________________________
G4KineticTrackVector* G4BertCascInterface::ConvertGenieSecondariesToG4(std::vector<GHepParticle> partList) const
{
...
//___________________________________________________________________________
  G4KineticTrackVector* G4BertCascInterface::ConvertGenieSecondariesToG4(GHepRecord* evrec) const
  {
...
//___________________________________________________________________________
bool G4BertCascInterface::Conserve4Momentum(GHepRecord* evrec) const
{
  GHepParticle* probe = evrec->Probe(); // incoming neutrino
...
//___________________________________________________________________________
void G4BertCascInterface::LoadConfig(void)
{
...
void G4BertCascInterface::Configure(const Registry & config)
{
...
//___________________________________________________________________________
void G4BertCascInterface::Configure(string param_set)
{
